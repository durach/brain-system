...
class configuration(agentAbstract.configuration):
	...
	def import_data(self):
		...
		for k, v in self.cp.items('lamps'):
			# Tables
			res = re.search('tables\.(?P<table>\d+)\.relay', k)
			if res:
				...
				continue
			# Start
			res = re.search('stand\.(?P<type>start|falsestart)\.relay', k)
			if res:
				matches = res.groupdict()
				try:
		...
		for k, v in self.cp.items('inputs'):
			# Tables
			res = re.search('tables\.(?P<table>\d+)\.input', k)
			if res:
				matches = res.groupdict()
				try:
					table_no = int(matches['table'])
...
class threadKEUSB24R(threading.Thread):
	...
	def init_port(self):
		self.device = interfaceKEUSB24R.interfaceKEUSB24R()
		if self.is_initialized():
